...
class Marketplace implements MarketplaceInterface 
  ...
  /**
    ...
    public function getbrandlist() {			
		...
		$result[]=array('status'=>array("code"=>"1","message"=>"Success"),'brandlist'=>array(array('id'=>1,'img'=>'http://btranz.website/fitness_logo4.jpg'),array('id'=>2,'img'=>'http://btranz.website/fitness_logo5.jpg')));
		...
		foreach($result3 as $data)
		{
				   ...
				   if($ratingSummary=='')
				{
				   ...
				   //print_r($ratingSummary);
					...
					$result4[]=array("id"=>$currentproduct->getId(),"name"=>$currentproduct->getName(),"description"=>$currentproduct->getDescription(),"price"=>$currentproduct->getPrice(),"finalPrice"=>$currentproduct->getFinalPrice(),"thumbnail"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getthumbnail(),"offferTag"=>array("shareurl"=>$producturl,"Tag"=>$optionText,'discount'=>$savetag,'rating'=>$ratingSummary),'wishlisttag'=>$wishlisttag);
		...
		$connection = $resource->getConnection();
		 ...
		 if(!empty($result_result))
		 {
		  ...
		  // $result4[]=array("id"=>$currentproduct->getId(),"name"=>$currentproduct->getName(),"description"=>$currentproduct->getDescription(),"price"=>$currentproduct->getPrice(),"finalPrice"=>$currentproduct->getFinalPrice(),"thumbnail"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getthumbnail(),"fullImage"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getImage(),'sku'=>$currentproduct->getSku(),'mediagallery'=>$imgarray,"offferTag"=>array("shareurl"=>$producturl,"Tag"=>$optionText,'discount'=>$savetag,'rating'=>$ratingSummary),'delivery'=>array(array('method'=>'Standard Delivery','cost'=>'Free'),array('method'=>'Expresee Delivery','cost'=>'$90')));
			 $result4[]=array("id"=>$currentproduct->getId(),"name"=>$currentproduct->getName(),"description"=>$currentproduct->getDescription(),"price"=>$currentproduct->getPrice(),"finalPrice"=>$currentproduct->getFinalPrice(),"thumbnail"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getthumbnail(),"fullImage"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getImage(),'sku'=>$currentproduct->getSku(),'mediagallery'=>$imgarray,"offferTag"=>array("shareurl"=>$producturl,"Tag"=>$optionText,'discount'=>$savetag,'rating'=>$ratingSummary,'totalreview'=>$totalreview),'delivery'=>array(array('method'=>'Standard Delivery','cost'=>'Free','subtitle'=>'Delivery in 5-6 days'),array('method'=>'Expresee Delivery','cost'=>'5','subtitle'=>'Delivery in 1-2 days')),'wishlisttag'=>$wishlisttag);
		     $result[]=array('status'=>array("code"=>"1","message"=>"sucess"),'productdetails'=>$result4);
		 }
		 else
		 {
    ...
    public function getcartproduct($userid) { 
		...
		if(!empty($result_result))
		{
			...
			$product_id=$data['product_id'];
		      ...
		      $result4[]=array("id"=>$product_id,"name"=>$currentproduct->getName(),"description"=>$currentproduct->getDescription(),"price"=>$currentproduct->getPrice(),"finalPrice"=>$currentproduct->getFinalPrice(),"thumbnail"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getthumbnail(),"fullImage"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getImage(),'sku'=>$currentproduct->getSku(),'mediagallery'=>$imgarray,'qty'=>$data['qty'],"offferTag"=>array("shareurl"=>$producturl,"Tag"=>$optionText,'discount'=>$savetag,'rating'=>$ratingSummary));
		    ...
		    //print_r($result_result); 
				...
				else
				{
										...
										/* $quote->save();
										  ...
										  $quoteItem = $this->quoteItemFactory->create();
										  echo $quoteItem_id= $quoteItem->getId();
										$result='success'; */
										 //$quoteItem = $this->quoteItemFactory->create();
										//echo $quoteItem_id= $quoteItem->getId();
										//echo "first";
...
/* 		    $quote_sql = "Select * FROM quote where customer_id=$userid and store_id=1 and is_active=1 order by entity_id ASC limit 1";
   ...
   /**
				  ...
				  if(!empty($result_wishlist))
				  {
					  foreach($result_wishlist as $wsh)
					  {
						  ...
						  //$wishlistproduct[]=$wsh['product_id'];
						   $wishlistproduct[]=array("id"=>$currentproduct->getId(),"wishlist_item_id"=>$wsh['wishlist_item_id'],"name"=>$currentproduct->getName(),"description"=>$currentproduct->getDescription(),"price"=>$currentproduct->getPrice(),"finalPrice"=>$currentproduct->getFinalPrice(),"thumbnail"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getthumbnail(),"fullImage"=>'http://btranz.website/pub/media/catalog/product'.$currentproduct->getImage(),'sku'=>$currentproduct->getSku(),"offferTag"=>array("shareurl"=>$producturl,"Tag"=>$optionText,'discount'=>$savetag,'rating'=>$ratingSummary));
					  }
	...
	{ 
			 ...
			 $resource = $objectManager->get('Magento\Framework\App\ResourceConnection');
		     $connection = $resource->getConnection();
			echo $quote_sql2 = "Select * FROM customer_entity where email='$email'";
				 $result_result2 = $connection->fetchAll($quote_sql2);
				 if(!empty($result_result2)):
...
/* 			$timestamp = strtotime($selecteddate);
	...
	/* $availableappointment='<td colspan="7">
	<div class="booked-appt-list shown" style="display: block;">
	<h2><span>Available Appointments on </span><strong>December 9, 2016</strong><span></span></h2>
	
	<div class="timeslot bookedClearFix" style="padding: 21px 3px 82px 8px;">
	<div style="
    float: left;
    width: 80%;
    ">
     <h3>
					<i class="fa fa-clock-o" aria-hidden="true"></i>
					 8:00 am – 9:00 am
					<h3>
					<h5>5 spaces available</h5>
    </div>
    <div style="float: right;">
          <div class="primary">
                <a href="#" class="action create primary"><span>Book Appointment</span></a>
            </div>
    </div>
    </div>
	<div class="timeslot bookedClearFix" style="padding: 21px 3px 72px 8px;">
	<div style="
    float: left;
    width: 80%;
    ">
 ...
 */
    ...
    public function getreview($product_id)
	{
		...
		$resource = $objectManager->get('Magento\Framework\App\ResourceConnection');
		$connection = $resource->getConnection();
		$quote_sql = "Select * FROM review where entity_pk_value=$product_id and status_id=1";
