...
class Edit extends \Adbanner\Gridpartbanner\Controller\Adminhtml\Template
{
    /**
     * Core registry
     *
    ...
    {
        $model = $this->_objectManager->create('Adbanner\Gridpartbanner\Model\Template');
        $id = $this->getRequest()->getParam('id');
        if ($id) {
            $model->load($id);
        }

        $this->_coreRegistry->register('_gridpartbanner_template', $model);

        $this->_view->loadLayout();
        $this->_setActiveMenu('Adbanner_Gridpartbanner::gridpartbanner_template');
