 ...
 */
class GetNewProducts extends \Custom\Chharo\Controller\ApiController
{
    /**
     * $_categoryTree.
     *
    ...
    public function execute()
    {
        if ($this->getRequest()->getPost()) {
            ...
            try {
                ...
                $newProductCollection = $this->_objectManager
                 ...
                 */
                $newProductCollection->addAttributeToFilter(
                    'status',
                    ['eq' => 1]
                );

                $newProductCollection->setVisibility($this->_productVisibility->getVisibleInSiteIds());

                $this
                    ->_objectManager
                    ->create('\Magento\CatalogInventory\Helper\Stock')
                    ->addInStockFilterToCollection($newProductCollection);
                ...
                foreach ($newProductCollection as $eachProduct) {
                    ...
                    if ($eachProduct->isAvailable()) {
                        $newProducts[] = $this->_helperCatalog
