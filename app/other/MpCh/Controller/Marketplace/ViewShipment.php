    ...
    public function execute()
    {
        if ($this->getRequest()->getPost()) {
            try {
                ...
                $shipmentId = $this->getRequest()->getPost("shipmentId");
                ...
                if (count($marketplaceOrders)) {
                    ...
                    $returnArray["subHeading"] = __("Shipment #%1 | %2", $shipment->getIncrementId(), $shipment->getCreatedAtStoreDate());
                    ...
                    if ($tracking->getTrackingNumber()) {
                        $returnArray["tracking"]["carierTitle"] = __("Carrier");
                        $returnArray["tracking"]["trackingTitle"] = __("Tracking Number");
                        $returnArray["tracking"]["carierValue"] =  $tracking->getCarrierName();
                        $returnArray["tracking"]["trackingValue"] =  $tracking->getTrackingNumber();
                    }
